...
public class Basic3dExampleCamera
{
    ...
    {
        var aspectRatio = graphicsDevice.Viewport.Width / (float)graphicsDevice.Viewport.Height;
        projectionMatrix = Matrix.CreatePerspectiveFieldOfView(fovInDegrees * ((3.14159265358f) / 180f), aspectRatio, .05f, 1000f);
    }
    ...
    {
        var state = Mouse.GetState(gameWindow);
        ...
        if (mouseLookIsUsed)
        {
            var diff = state.Position.ToVector2() - mState.Position.ToVector2();
            if (diff.X != 0f)
    ...
    public void RotateUp(GameTime gameTime)
    {
        ...
        var matrix = Matrix.CreateFromAxisAngle(camerasWorld.Right, MathHelper.ToRadians(radians));
    ...
    {
        var radians = RotationRadiansPerSecond * (float)gameTime.ElapsedGameTime.TotalSeconds;
        var matrix = Matrix.CreateFromAxisAngle(camerasWorld.Up, MathHelper.ToRadians(radians));
        LookAtDirection = Vector3.TransformNormal(LookAtDirection, matrix);
        ReCreateWorldAndView();
    }
